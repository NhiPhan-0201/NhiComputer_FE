...
export class ProductImageUploadDialogComponent implements OnInit {
  ...
  // Hàm lưu hình ảnh
  save() {
    if (this.postForm.valid && this.fileToUpload) {
      const productId = this.postForm.value.productId;
      this.productImageService.upload(productId, this.fileToUpload).subscribe(data => {
        this.modalService.dismissAll();
        this.toastr.success('Tải ảnh thành công!', "Hệ thống");
        this.saveFinish.emit('done');
        this.resetForm();
      }, error => {
        this.toastr.error('Tải ảnh thất bại! ' + error, 'Hệ thống');
      });
    } else {
      this.toastr.error('Vui lòng chọn sản phẩm và hình ảnh!', 'Hệ thống');
    }
  }
  ...
  resetForm() {
    ...
    this.url = ''; // Reset preview ảnh
    this.fileToUpload = null; // Reset file đã chọn
  }
  ...
  readUrl(event: any) {
    if (event.target.files && event.target.files[0]) {
      ...
      reader.onload = (e: any) => {
        ...
        this.postForm.patchValue({ imageUrl: this.url });
      };
